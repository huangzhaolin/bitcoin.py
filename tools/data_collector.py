...
class DataCollectorFactory(object):
     ...
     def trade_data_handle(self,json_data,orignal="BITCCHINA"):
          ...
          for data in json_data:
               tradeData=TradeData(zip(["datetime","price","num","orignal","trade_num","trade_id"],[time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(data.get("date","")))),data.get("price",""),data.get("amount",""),orignal,0,data.get("tid","")]))
